...
function handle_gpt_request() {
    if ( ! is_user_logged_in() ) {
        wp_send_json_error( [ 'message' => 'Требуется авторизация.' ], 403 );
        exit;
    }
        ...
        if (is_readable($path)) {
            ...
            switch ($ext) {
                case 'txt': case 'md': case 'json': case 'csv': case 'xml':
                    $source_text = file_get_contents($path);
                    break;
                case 'docx':
                    // С помощью PhpWord читаем .docx
                    try {
                    ...
                    break;
                case 'pdf':
                    // С помощью PdfParser читаем PDF
                    try {
                        $parser = new Parser();
                        $pdf    = $parser->parseFile($path);
                        $source_text = $pdf->getText();
                    } catch (Exception $e) {
                        $source_text = '';
                    }
                    break;
                default:
                    // Прочие форматы читаем как текст
                    $source_text = file_get_contents($path);
                    break;
            }
        }
    ...
    $resp = wp_remote_post('https://open-api-js-proxy.onrender.com/proxy', [
        ...
        'body'    => wp_json_encode(['model'=>'gpt-4o-mini','messages'=>$messages]),
        'timeout' => 20
    ]);

    // 2) Единая ошибка на любые неуспехи раннего уровня
    if ( is_wp_error( $resp ) ) {
    ...
    // 3) Только теперь списываем запрос и возвращаем ответ
